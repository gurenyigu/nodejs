...
var serv = http.createServer(function(req, res) {
  ...
  if ('/' == req.url) {
    ...
    res.end(["<head><meta charset = 'utf-8'></head>", '<h1>填写表格后提交</h1>', "<form method = 'POST' action = '/label'>",
      "<fieldset><label>填写表单啊</label><p>Name</p><input type = 'text' name = 'name'><button>Submit</button></form>"
    ].join(' '));
    // 请求结束发送
    ...
    req.on('end', function() {
      ...
      // 指定头编码 utf-8  否则会 麻将文
      res.end(["<!DOCTYPE html><head><meta charset = 'utf-8'></head><body>", '<h3> method =<em>' + req.method + '</em> ' + key + '=' + query.parse(data)['name'] + '</h3></body></html>'].join(' '));
